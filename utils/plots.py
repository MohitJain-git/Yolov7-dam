...
def extract(image ,left, bottom, right, top):
    plt.ioff()
    fig = plt.figure(figsize=(6.4,6.4))
    ax1 = plt.subplot(1, 1, 1)
    plt.imshow(image[bottom:top, left:right])
    fig.add_subplot(ax1)
    fig.savefig("temp/x.jpg")
    plt.close(fig)
    path ="temp/x.jpg"
    return path

def plot_one_box(data, x, img, conf, color=None, label=None, line_thickness=4):

    # Plots one bounding box on image img
    tl = round(0.002 * (img.shape[0] + img.shape[1]) / 2) # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1,c2 = (int(x[0]), int(x[1])) , (int(x[2]), int(x[3]))

    path= extract(img,int(x[0]), int(x[1]) , int(x[2]), int(x[3]))
    if(label == None):
        label = 'Person'
    label = str(label).capitalize()
    
    if(label == 'Person' and conf >= 0.80):
        label = recognize(label, path, data)
    elif(label == 'Person' and conf < 0.80):
        return []
